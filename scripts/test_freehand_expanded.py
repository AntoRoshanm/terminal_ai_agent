...
sess = ss.create_session("Freehand Expanded Test")
...
# 1. Hardware query
freehand("show me how much ram memory is free right now")

# 2. Ambiguous query (B.0.2 stress-test)
freehand("clean up my system")

# 3. Conversational query (B.0.2 stress-test)
freehand("can you help me check if python is installed")

# 4. File creation
freehand("create a file test_log.txt on Desktop with 'test 123'")

# 5. OS query
freehand("what is the name and version of this operating system")

# Clean up test_log.txt if created
log_file = Path.home() / "Desktop" / "test_log.txt"
if log_file.exists():
    log_file.unlink()
    print("[GROUND TRUTH] Cleaned up Desktop/test_log.txt", flush=True)
